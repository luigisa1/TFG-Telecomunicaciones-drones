...
import time
IP = "192.168.110.3"  # Camera IP address
PORT = 80  # Port
USER = "admin"  # Username
PASS = "DIADRON1234"  # Password
...
class ptzControl(object):
    ...
    def move_abspantilt(self, pan, tilt, velocity,zoom_actual):
    ...
    def move_relative(self, pan, tilt, velocity_pan,velocity_tilt):
        ...
        self.requestr.Speed.PanTilt.x = velocity_pan
        self.requestr.Speed.PanTilt.y = velocity_tilt
        self.requestr.Speed.Zoom = 0
        ret = self.ptz.RelativeMove(self.requestr)

    # Relative move functions --NO ERRORS BUT CAMERA DOES NOT MOVE
    def move_relative_2(self, pan, tilt, velocity_pan,velocity_tilt):
        self.requestr.Translation.PanTilt.x = pan
        self.requestr.Translation.PanTilt.y = tilt
        self.requestr.Translation.Zoom.x=0
        #self.requestr.Speed.PanTilt = [velocity,velocity]
    ...
    def wait_for_move_completion(self):
        while True:
            ...
            if not status.Position.PanTilt.space:
                # La operación de movimiento se ha completado
                break
        
    def move_relative_zoom(self, pan, tilt, velocity_pan,velocity_tilt,zoom,velocity_zoom):
        #t=time.time()
        self.requestr.Translation.PanTilt.x = pan
        self.requestr.Translation.PanTilt.y = tilt
        ...
        self.requestr.Speed.PanTilt.x = velocity_pan
        self.requestr.Speed.PanTilt.y = velocity_tilt
        self.requestr.Speed.Zoom =velocity_zoom
        ret = self.ptz.RelativeMove(self.requestr)
        #t1=time.time()-t
        #print("------------------------------Mando movimiento-----------------------")
    ...
    def zoom_relative(self, zoom, velocity):
        ...
        self.requestr.Speed.Zoom = velocity
        ret = self.ptz.RelativeMove(self.requestr)
    ...
    #OBTENER ZOOM, COMPROBAR SI FUNCIONA
    def get_current_zoom(self):
        current_zoom = self.ptz.GetStatus({'ProfileToken': self.media_profile.token}).Position.Zoom.x
        return current_zoom
    
    def get_current_pan_tilt(self):
        # Obtener el estado actual de la cámara
        status = self.ptz.GetStatus({'ProfileToken': self.media_profile.token})
        # Obtener los valores de pan y tilt desde el estado
        current_pan = status.Position.PanTilt.x
        current_tilt = status.Position.PanTilt.y
        return current_pan, current_tilt
    
    def get_status(self):
        status= self.ptz.GetStatus({'ProfileToken': self.media_profile.token}).MoveStatus.PanTilt
        return status