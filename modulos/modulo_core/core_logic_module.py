...
import threading
import time
from ..camaras_fijas.camera_detector import CameraDetector
from ..calculos import calc_dist as cd
...
class CoreLogicModule:
    ...
    """
    def __init__(self, comunicacion_sdr, ptz_barrido, camaras, ptz_detector, intersection_handler, camera_capture, model, size, classes, sdr, transmisor, flag_servidor, enviar):
        ...
        self.flag_servidor = flag_servidor
        self.enviar= enviar
        self.barrido_inicial = False
        self.flag_enviar_alarma = True
    ...
    def iniciar_camaras_fijas(self):
        for camera in self.camaras[:-1]:
            ...
            detector = CameraDetector(camera, self.intersection_handler.cola_desviaciones, self.model, self.size, self.classes)
    ...
    def comprobar_estado(self):
        ...
        """
        if self.ptz_detector.detection or self.intersection_handler.detections > 0:
    ...
    def comprobar_intersection_handler(self):
        ...
        """
        # if self.intersection_handler.move_PTZ is not None and self.ptz_detector.detection==False and self.intersection_handler.aviso_cruce==False:
        if self.intersection_handler.move_PTZ is not None and not self.ptz_detector.detection and self.intersection_handler.aviso_cruce==False:
    ...
    def comprobar_aviso(self):
        ...
        if self.estado=="Dron localizado" and self.aviso_deteccion==False:
            ...
            #Envio vídeo
            # if self.flag_servidor:
            if self.flag_enviar_alarma and self.enviar:
                self.flag_enviar_alarma = False
    ...
    def enviar_alarma_cra(self):
        ...
        for i in range(3):
            print("Buscando cámaras con detección, iteración: ",i)
            if self.ptz_detector.detection:
                self.camera_capture.iniciar_captura(self.camaras[-1].source, 10)
                break
            elif self.intersection_handler.detections > 0:
                for camera in self.camaras[:-1]:
                    if camera.detection:
                        self.camera_capture.iniciar_captura(camera.source, 10)
                        break
                else:
                    # Si no se rompe el bucle interno, continuamos con el bucle externo.
                    continue
                # Si se rompe el bucle interno, también rompemos el bucle externo.
                break
            time.sleep(0.5)
        ...
        if self.estado=="Dron localizado":
            ...
            if self.intersection_handler.punto_dron is not None:  
                ...
                if self.enviar: self.transmisor.enviarAlarma(video_path = "/home/diadron/ultralytics/video_detect/output.mp4", x = self.intersection_handler.punto_dron[0], y = self.intersection_handler.punto_dron[1], z = self.intersection_handler.punto_dron[2])
            
            else:
                if distancia==0:
                    print("Envío solo el video porque el SDR no sabe la distancia actual al dron")
                    if self.enviar: self.transmisor.enviarAlarma(video_path = "/home/diadron/ultralytics/video_detect/output.mp4", x = 0, y = 0, z = 0)
                    ...
                    if self.ptz_detector.detection:
                        ...
                        if self.enviar: self.transmisor.enviarAlarma(video_path = "/home/diadron/ultralytics/video_detect/output.mp4", x = punto_medio_final[0], y = punto_medio_final[1], z = punto_medio_final[2])
                    elif self.intersection_handler.detections > 0:
                        for camera in self.camaras[:-1]:
                            if camera.detection:
                                punto_medio_final, _, _ = cd.intersectar_recta_esfera(origen_SDR, distancia, camera.position, camera.orientation[0], camera.orientation[1])
                                #Enviamos el video con las coordenadas calculadas por el cruce de informaciones de la cámara y el SDR
                                print("Se envía el video junto con el punto calculado por el cruce de informaciones ", punto_medio_final)
