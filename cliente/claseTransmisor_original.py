...
class TransmisorSIA:
    ...
    def __start_maintenance_monitor(self):
        self.maintenance_thread = threading.Thread(target=self.__monitor_mantenimiento, daemon=True)
    ...
    def enviarInicioSistema(self):
        ...
        try:
            ...
            print(f"Error al enviar mensaje de inicio de sistema: {e}")
            return False
...
#---------------------------------------------------------------
    ...
    def __socketSSL_UDP(self):
        ...
        while intentos < 3:  # Limita los intentos de handshake a 3
            try:
                ...
                clientSock.sendto(
                    b'Hello', (self.serverAddress, self.serverPort))
                #print("Mensaje inicial enviado, esperando handshake...")

                # Realizar handshake con timeout
                thread = threading.Thread(
                    target=self.__esperaHandshake, args=(clientSock_seguro,))
                thread.start()
                thread.join(timeout)
                ...
                if intentos >= 3:
                    clientSock.close()
                    raise ErrorDeHandshake(
                        "No se pudo completar el handshake después de varios intentos.")
                continue
    ...
    def __generar_mensaje_sia(self, cifrado_flag, protocolo, secNum, Rrcvr, Lpref, acct, mensajeContactID, xdata, timestamp):
        ...
        # Caso cifrado
            ...
            # Se añade el timestamp
            mensajeSIA_padding += f"_{timestamp}"
    ...
    def __esperaHandshake(self, clientSock_seguro):
